...
export function WeatherChip() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const body = (await response.json()) as WeatherState & { error?: string };
      if (!response.ok) {
        setWeather({
          status: "error",
  ...
  const useDeviceLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setSettingsError("Could not get your location. Check browser permissions.");
        setSaving(false);
      },
    ...
    <div className="relative">
      <div className="relative flex min-h-[60px] min-w-[148px] items-center gap-3 rounded-[20px] bg-white px-5 py-4 shadow-sm">
        {weather.status === "loading" && (
          <>
            ...
            <div>
              ...
              <p className="text-lg font-medium text-stone-400">…</p>
            </div>
          </>
        )}
        ...
        {weather.status === "no_location" && (
          <>
            <WeatherIcon bucket="partly_cloudy" isDay className="h-10 w-10 opacity-40" />
            <div>
              <p className="text-sm text-stone-500">Weather</p>
              <p className="text-sm font-medium text-stone-600">Set location</p>
              <p className="text-xs text-stone-400">Tap ⚙ to configure</p>
            </div>
          </>
        )}
        ...
        {weather.status === "error" && (
          <>
            <WeatherIcon bucket="cloudy" isDay className="h-10 w-10 opacity-50" />
            <div>
              <p className="text-sm text-stone-500">Weather</p>
              <p className="text-sm font-medium text-stone-600">Unavailable</p>
            </div>
          </>
        )}
        ...
        <button
          ...
          className="absolute top-2 right-2 flex h-8 w-8 items-center justify-center rounded-lg text-stone-400 opacity-40 transition hover:bg-stone-100 hover:text-stone-700 hover:opacity-100"
      ...
      {settingsOpen && (
        ...
        >
          <p className="text-sm font-semibold text-stone-900">Weather location</p>
          ...
          <div className="mt-4 space-y-3">
            <button
              ...
              className="min-h-[44px] w-full rounded-xl border border-stone-200 bg-[#FAF8F3] px-3 py-2 text-left text-sm font-medium text-stone-800 transition hover:border-[#F4B400]/50 disabled:opacity-50"
            >
            ...
            <form onSubmit={submitZip} className="space-y-2">
              ...
              <input
                ...
                className="min-h-[44px] w-full rounded-xl border border-stone-200 px-3 text-sm focus:border-[#F4B400] focus:outline-none focus:ring-2 focus:ring-[#F4B400]/30"
              />
              <button
                type="submit"
                disabled={saving || !zip.trim()}
                className="min-h-[44px] w-full rounded-xl bg-[#F4B400] text-sm font-semibold text-stone-900 transition hover:bg-[#e0a800] disabled:opacity-50"
              >
                {saving ? "Saving…" : "Save"}
              </button>
            </form>
