...
/** Transparent favicon-style bee mark (same paths as `src/app/icon.svg`). */
...
type AppBrandLockupProps = {
  className?: string;
  /** Bee mark size in px. Default 24 for app chrome. */
  markSize?: number;
};

/** Subtle app-chrome lockup: SVG bee + styled BumbleHub text. */
export function AppBrandLockup({
  ...
  return (
    <span
      className={`inline-flex items-center font-[family-name:var(--font-bricolage)] ${className}`}
      style={{ gap: 8 }}
    >
      <BeeMark size={markSize} />
      <span
